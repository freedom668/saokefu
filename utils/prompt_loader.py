...
def load_rag_prompts():
    try:
        rag_prompt_path = get_abs_path(prompts_conf['rag_summarise_prompt_path'])
    except KeyError as e:
        logger.error(f"[load_rag_prompt]在yaml配置项中没有rag_summarise_prompt_path配置项")
        raise e

    try:
        return open(rag_prompt_path,'r',encoding='utf-8').read()
    except Exception as e:
        logger.error(f"[load_rag_prompt]解析RAG总结出错,{str(e)}")
        raise e
